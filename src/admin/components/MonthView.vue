    ...
    <div class="shadow ring-1 ring-black ring-opacity-5 lg:flex lg:flex-auto lg:flex-col">
        ...
        <div class="flex bg-gray-200 text-xs leading-6 text-gray-700 lg:flex-auto">
            <div class="hidden w-full lg:grid lg:grid-cols-7 lg:grid-rows-6 lg:gap-px">
                <div v-for="day in CalendarModule.days" :key="day.date" :class="[day.isCurrentMonth ? 'bg-white' : 'bg-gray-50 text-gray-500', 'relative py-2 px-3']">
                    ...
                    <ol v-if="day.events.length > 0" class="mt-2">
                        <li v-for="event in day.events.slice(0, 2)" :key="event.id">
                            <a @click="edit(event.id)" class="group flex cursor-pointer">
                                <p class="flex-auto truncate font-medium text-gray-900 group-hover:text-indigo-600">
                                    {{ event.name }}
                                </p>
                                <!-- <time :datetime="event.dateStart" class="ml-3 hidden flex-none text-gray-500 group-hover:text-indigo-600 xl:block">{{ event.dateStart }}</time> -->
                                <div class="ml-3 hidden flex-none text-gray-500 group-hover:text-indigo-600 xl:block">{{ event.count }}</div>
                            </a>
                        </li>
            ...
            <div class="isolate grid w-full grid-cols-7 grid-rows-6 gap-px lg:hidden">
                <button
                    v-for="day in CalendarModule.days"
                    :key="day.date"
                    type="button"
                    :class="[
