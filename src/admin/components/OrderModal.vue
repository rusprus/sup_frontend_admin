                    ...
                    >
                        <div>
                            <form class="space-x-2 z-10">
                                ...
                                <div class="space-x-2 z-10 grid md:grid-cols-2">
                                    ...
                                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                        ...
                                        <div class="mt-1 sm:mt-0 sm:col-span-2">
                                            <!-- <input
                                                type="datetime"
                                                v-model="orders.order.dateStart"
                                                name="last-name"
                                                id="last-name"
                                                autocomplete="family-name"
                                                class="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                                            /> -->
                                            <Datepicker v-model="orders.order.dateStart"></Datepicker>
                                        </div>
                                    </div>

                                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                        <label for="last-name" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2"> Аренда до </label>
                                        <div class="mt-1 sm:mt-0 sm:col-span-2">
                                            <!-- <input
                                                type="datetime-local"
                                                v-model="orders.order.dateEnd"
                                                name="last-name"
                                                id="last-name"
                                                autocomplete="family-name"
                                                class="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md"
                                            /> -->
                                            <Datepicker v-model="orders.order.dateEnd" ></Datepicker>
                                        </div>
                                    </div>
                                    ...
                                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                        ...
                                        <div class="mt-1 sm:mt-0 sm:col-span-2">
                                            ...
                                            /> -->
                                            <select v-model="orders.order.sup_id" class="block max-w-lg w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md">
    ...
    methods: {
        ...
        save() {
            if (this.orders.order.id !== null) {
                this.updateOrder({
                    id: this.orders.order.id,
                    name: this.orders.order.name,
                    dateStart: this.orders.order.dateStart,
                    dateEnd: this.orders.order.dateEnd,
                    sup_id: this.orders.order.sup_id,
                    status: this.orders.order.status,
                    note: this.orders.order.note,
                    // count: this.orders.order.count,
                });
            } else {
                this.addOrder(this.orders.order);
            }
            this.toggleModule(false);
        },
        delOrder() {
